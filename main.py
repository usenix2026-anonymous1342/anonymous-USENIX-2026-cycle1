...
from utils import get_dataset, get_model, get_optimizer, get_active_wm
from utils import save_passive_target, get_passive_target, get_passive_wm_dataset, get_passive_wm_test_dataset, get_memory_dataset 
from utils import AvgMeter, extract_features
...
class VFL_framwork(object):
    ...
    def setup(self):
        ...
        # Dataset
        split_train_dataset, split_test_dataset, split_wm_test_dataset = get_dataset(self.cfg)
        ...
        self.bottom_model_optimizer_list = [get_optimizer(bottom_model.parameters(), **self.cfg.optimizer) for
                                              bottom_model in self.bottom_model_list]
        self.top_model_optimizer = get_optimizer(self.top_model.parameters(), **self.cfg.optimizer)

        # Load Pre-trained Watermarked Model
        if self.cfg.pretrain == True:
            save_path = os.path.join('./model/', f"{self.cfg.dataset.name}")
    ...
    def active_setup(self):
        self.active_wm = get_active_wm(self.cfg)
    ...
    def passive_setup(self):
        ...
        # Dataset
        self.memory_dataset = get_memory_dataset(self.cfg)
        self.passive_wm_dataset = get_passive_wm_dataset(self.cfg)
        self.passive_wm_test_dataset = get_passive_wm_test_dataset(self.cfg)
        ...
        self.target_data, self.target_transform = get_passive_target(self.cfg)
    ...
    def train(self, round=0):
        ...
        # Extract the top 5 images with highest gradient scores for each passive party
        if round < 5:
            save_passive_target(self.cfg, top_heap_list)
    ...
    def global_wm_test(self):
        ...
        self.wm_acc_history.append(self.wm_acc_meter.get())
    ...
    def passive_wm_test(self):
        for idx in range(len(self.bottom_model_list)):
            train_features, train_labels = extract_features(self.bottom_model_list[idx], self.memory_loader, idx)
            passive_wm_features, passive_wm_labels = extract_features(self.bottom_model_list[idx], self.passive_wm_test_loader, idx)
    ...
    def save(self):
        save_path = os.path.join('./model/test', f"{self.cfg.dataset.name}")
        for idx in range(len(self.bottom_model_list)):
            passive_model_path = os.path.join(save_path, f"{cfg.dataset.passive_client_num}-{idx+1}_passive_model.pth")
            # torch.save(self.bottom_model_list[idx].state_dict(), passive_model_path)
