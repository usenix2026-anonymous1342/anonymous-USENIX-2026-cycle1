...
from utils import get_passive_target, get_passive_wm_dataset, get_passive_wm_test_dataset, get_memory_dataset 
from utils import AvgMeter, extract_features, extract_util_features
...
def save_passive_target(cfg, top_heap_list):
    for idx, top_heap in enumerate(top_heap_list):
        ...
        save_path = f"./watermark/fidelity_complete/{cfg.dataset.name}/party_{idx}_of_{cfg.dataset.passive_client_num}/"
...
class VFL_framwork(object):
    ...
    def passive_wm_test(self):
        for idx in range(len(self.bottom_model_list)):
            train_features, train_labels = extract_util_features(self.bottom_model_list[idx], self.memory_loader, idx)
...
if __name__ == '__main__':
    from config_fidelity_complete import config as cfg
