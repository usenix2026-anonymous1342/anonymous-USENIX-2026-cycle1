...
class VFL_framwork(object):
    ...
    def finetune(self):
        ...
        for init_x, batch_x, batch_y in tqdm(self.train_loader):
            ...
            output_tensor_bottom_model_list = [self.bottom_model_list[idx](x_a[idx]) 
                                               for idx in range(len(self.bottom_model_list))]
            
            # TOP-Model Infer and Update
            input_tensor_top_model_a_list = output_tensor_bottom_model_list
            ...
            # Backward pass only for bottom models
            for idx in range(len(self.bottom_model_list)):
...
if __name__ == '__main__':
    ...
    result_path = f"./results/{cfg.dataset.name}/passive_finetune.csv"
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f'./results/{cfg.dataset.name}/passive_finetune.png', dpi=300)