...
def add_dp_noise(tensor, noise_scale):
    noise = torch.normal(mean=0, std=noise_scale, size=tensor.size(), device=tensor.device)
    return tensor + noise
...
def save_passive_target(cfg, top_heap_list):
    for idx, top_heap in enumerate(top_heap_list):
        ...
        save_path = os.path.join(cfg.passive_wm.target_path, f"party_{idx}_of_{cfg.dataset.passive_client_num}/")
...
class VFL_framwork(object):
    ...
    def setup(self):
        ...
        if self.cfg.pretrain == True:
            ...
            top_model_path = os.path.join(save_path, f'DP_{cfg.dataset.passive_client_num}_active_model.pth')
            ...
            for idx, model in enumerate(self.bottom_model_list):
                passive_model_path = os.path.join(save_path, f"DP_{cfg.dataset.passive_client_num}-{idx+1}_passive_model.pth")
    ...
    def train(self, round=0):
        ...
        for init_x, batch_x, batch_y in tqdm(self.train_loader):
            ...
            output_tensor_bottom_model_list = [
                add_dp_noise(self.bottom_model_list[idx](x_a[idx]), noise_scale=0.1)
    ...
    def save(self):
        save_path = os.path.join('./model/ablation_dp', f"{self.cfg.dataset.name}")
...
if __name__ == '__main__':
    ...
    from config_ablation_dp import config as cfg
    ...
    if cfg.pretrain is False:
        ...
        result_path = f"./results/{cfg.dataset.name}/ablation_dp_train.csv"
        os.makedirs(os.path.dirname(result_path), exist_ok=True)

        with open(result_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["Round", "W-Acc", "G-WSR", "A-WSR"])
        ...
        plt.legend()
        plt.grid()
        plt.savefig(f'./results/{cfg.dataset.name}/ablation_dp_train.png', dpi=300)
    ...
    rounds = range(0, len(w_acc_list))
    result_path = f"./results/{cfg.dataset.name}/ablation_dp_embed.csv"
    os.makedirs(os.path.dirname(result_path), exist_ok=True)
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f'./results/{cfg.dataset.name}/ablation_dp_embed.png', dpi=300)