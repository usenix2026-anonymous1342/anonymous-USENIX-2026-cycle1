...
def random_mask(tensor, sparsity):
...
class VFL_framwork(object):
    ...
    def setup(self):
        ...
        if self.cfg.pretrain == True:
            ...
            top_model_path = os.path.join(save_path, f'Sparse_{cfg.dataset.passive_client_num}_active_model.pth')
            ...
            for idx, model in enumerate(self.bottom_model_list):
                passive_model_path = os.path.join(save_path, f"Sparse_{cfg.dataset.passive_client_num}-{idx+1}_passive_model.pth")
    ...
    def train(self, round=0):
        ...
        for init_x, batch_x, batch_y in tqdm(self.train_loader):
            ...
            output_tensor_bottom_model_list = [
                random_mask(self.bottom_model_list[idx](x_a[idx]), sparsity=0.3)
    ...
    def save(self):
        save_path = os.path.join('./model/ablation_sparse', f"{self.cfg.dataset.name}")
...
if __name__ == '__main__':
    ...
    from config_ablation_sparse import config as cfg
    ...
    if cfg.pretrain is False:
        ...
        result_path = f"./results/{cfg.dataset.name}/ablation_sparse_train.csv"
        ...
        plt.legend()
        plt.grid()
        plt.savefig(f'./results/{cfg.dataset.name}/ablation_sparse_train.png', dpi=300)
    ...
    rounds = range(0, len(w_acc_list))
    result_path = f"./results/{cfg.dataset.name}/ablation_sparse_embed.csv"
    os.makedirs(os.path.dirname(result_path), exist_ok=True)
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f'./results/{cfg.dataset.name}/ablation_sparse_embed.png', dpi=300)